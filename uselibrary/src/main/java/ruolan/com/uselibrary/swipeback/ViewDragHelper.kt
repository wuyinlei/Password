...
private constructor(context: Context, private val mParentView: ViewGroup?, private val mCallback: Callback?) {
    ...
    fun flingCapturedView(minLeft: Int, minTop: Int, maxLeft: Int, maxTop: Int) {
        ...
        mScroller.fling(capturedView!!.left, capturedView!!.top,
                VelocityTrackerCompat.getXVelocity(mVelocityTracker!!, activePointerId).toInt(),
                VelocityTrackerCompat.getYVelocity(mVelocityTracker!!, activePointerId).toInt(), minLeft,
    ...
    protected fun canScroll(v: View, checkV: Boolean, dx: Int, dy: Int, x: Int, y: Int): Boolean {
        if (v is ViewGroup) {
            ...
            for (i in count - 1 downTo 0) {
                ...
                if (x + scrollX >= child.left && x + scrollX < child.right && y + scrollY >= child
                                .top && y + scrollY < child.bottom && canScroll(child, true, dx, dy,
                                x + scrollX - child.left, y + scrollY - child.top)) {
                    return true
                }
            }
        }
    ...
    private fun checkNewEdgeDrag(delta: Float, odelta: Float, pointerId: Int, edge: Int): Boolean {
        ...
        if (mInitialEdgeTouched!![pointerId] and edge != edge
    ...
    private fun releaseViewForPointerUp() {
        ...
        val xvel = clampMag(VelocityTrackerCompat.getXVelocity(mVelocityTracker!!, activePointerId),
                minVelocity, mMaxVelocity)
        val yvel = clampMag(VelocityTrackerCompat.getYVelocity(mVelocityTracker!!, activePointerId),
                minVelocity, mMaxVelocity)
     ...
     * [com.github.obsessive.library.swipeback.ViewDragHelper.Callback.getOrderedChildIndex]
     * .
     *
     * @param x X position to test in the parent's coordinate system
     * @param y Y position to test in the parent's coordinate system
    ...
    private fun getEdgeTouched(x: Int, y: Int): Int {
        ...
        if (x < mParentView!!.getLeft() + edgeSize) result = EDGE_LEFT
        if (y < mParentView!!.getTop() + edgeSize) result = EDGE_TOP
        if (x > mParentView!!.getRight() - edgeSize) result = EDGE_RIGHT
        if (y > mParentView!!.getBottom() - edgeSize) result = EDGE_BOTTOM
